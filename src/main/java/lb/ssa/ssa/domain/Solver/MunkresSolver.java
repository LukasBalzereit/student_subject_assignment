...
public class MunkresSolver {
    ...
    private int dummyRows = 0; //amount of rows added to make the array square (see makeSquare())
    private int[][] solvingArray/*array used to compute solution(gets changed)*/, comparisonArray;//the original array
    ...
    private void createLabels() {
        ...
        Arrays.setAll(columnLabel, i -> String.valueOf(i++));
        rowLabel = new String[solvingArray.length];
        Arrays.setAll(rowLabel, i -> String.valueOf(i++));
    }
    ...
    private Result createSolution() {
        ...
        if ((originalLength != solvingArray.length) && (solvingArray[0].length == columnLabel.length)) {
    ...
    private int[][] makeSquare(int[][] array) {
        ...
        if (columns == rows)
            ...
            throw new IllegalArgumentException("Array has more rows then columns");

        int[][] squareArray = new int[max][max];
        //fill empty elements inside row to get each row to an equal length (max)
        IntStream.range(0, rows)
                .forEach(i -> {
                    squareArray[i] = Arrays.copyOf(array[i], max); //empty elements will be filled with 0s (see Arrays API)
                });
        ...
        for (int iRow = 0; iRow < solvingArray.length; iRow++) {
            for (int iCol = 0; iCol < solvingArray[0].length; iCol++) {
                ...
                if (solvingArray[iRow][iCol] == 0) {
                    if ((!rowContains(iRow, STARRED)) && (!columnContains(iCol, STARRED))) { //TODO: eigene methoden Row/Column-ContainsStarred?
